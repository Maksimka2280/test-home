...
export default function PriceModal() {
  ...
  const handleCheckboxChange2 = (currencyId: number) => {
    setSelectedCurrency(currencyId === selectedCurrency ? null : currencyId);
  };
    ...
    <>
      ...
      >
        <span className="text-md font-normal text-gray-800 hover:text-blue-600 focus:outline-none">
          Цена до, {selectedCurrency ? currencySymbols[selectedCurrency] : '₽'}
        </span>
      </button>
        ...
        <div className="flex justify-center gap-4 mb-5 mt-[20px]">
          <div className="relative">
            <input
              ...
              className="w-[150px] rounded-[30px] bg-[#F3F3F3] border-none h-[40px] pl-[14px] pr-[30px] text-[#9D9D9D] focus:outline-none focus:ring-0"
            />
            {selectedCurrency && (
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[#9D9D9D]">
            ...
            {[
              ...
              <li key={currency.id} className="flex items-center w-full">
                <label htmlFor={currency.id.toString()} className="flex items-center cursor-pointer">
                  ...
                  />
                  <div
                    className="w-[17px] h-[17px] flex items-center justify-center rounded-[5px] bg-[#F3F3F3] transition-all duration-300 mr-[8px]"
                    onClick={() => handleCheckboxChange2(currency.id)} // Добавил onClick
                  >
